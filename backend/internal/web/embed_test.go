...
func TestInjectSiteTitle(t *testing.T) {
	t.Run("replaces_title_with_site_name", func(t *testing.T) {
		html := []byte(`<html><head><title>Tabro - AI API Gateway</title></head><body></body></html>`)
		...
		assert.NotContains(t, string(result), "Tabro")
	})
	...
	t.Run("returns_unchanged_when_title_has_attributes", func(t *testing.T) {
		...
		html := []byte(`<html><head><title lang="en">Tabro</title></head><body></body></html>`)
	...
	t.Run("preserves_rest_of_html", func(t *testing.T) {
		html := []byte(`<html><head><meta charset="UTF-8"><title>Tabro</title><script src="app.js"></script></head><body><div id="app"></div></body></html>`)
