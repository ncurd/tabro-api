    ...
    <div class="flex flex-1 flex-col p-4">
      ...
      <div class="mb-3 flex items-start justify-between gap-2">
        ...
        <div class="shrink-0 text-right">
          <div class="flex items-baseline gap-1">
            <span :class="['text-2xl font-extrabold tracking-tight', textClass]">{{ formatCredits(plan.price) }}</span>
          </div>
          <span class="text-[11px] text-gray-400 dark:text-dark-500">/ {{ validitySuffix }}</span>
          <div v-if="plan.original_price" class="mt-0.5 flex items-center justify-end gap-1.5">
            <span class="text-xs text-gray-400 line-through dark:text-dark-500">{{ formatCredits(plan.original_price) }}</span>
      ...
      <div class="mb-3 grid grid-cols-2 gap-x-3 gap-y-1 rounded-lg bg-gray-50 px-3 py-2 text-xs dark:bg-dark-700/50">
        ...
        <div v-if="plan.daily_limit_usd != null" class="flex items-center justify-between">
          ...
          <span class="font-medium text-gray-700 dark:text-gray-300">{{ formatCredits(plan.daily_limit_usd) }}</span>
        </div>
        <div v-if="plan.weekly_limit_usd != null" class="flex items-center justify-between">
          <span class="text-gray-400 dark:text-dark-500">{{ t('payment.planCard.weeklyLimit') }}</span>
          <span class="font-medium text-gray-700 dark:text-gray-300">{{ formatCredits(plan.weekly_limit_usd) }}</span>
        </div>
        <div v-if="plan.monthly_limit_usd != null" class="flex items-center justify-between">
          <span class="text-gray-400 dark:text-dark-500">{{ t('payment.planCard.monthlyLimit') }}</span>
          <span class="font-medium text-gray-700 dark:text-gray-300">{{ formatCredits(plan.monthly_limit_usd) }}</span>
        </div>
