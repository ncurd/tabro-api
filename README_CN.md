...
Tabro 由 Sub2API 分叉而来，是一个 AI API 网关平台，用于分发和管理 AI 产品订阅的 API 配额。用户可以通过平台生成的 API Key 访问上游 AI 服务，而平台负责鉴权、计费、负载均衡和请求转发。

> **使用本项目前请仔细阅读：**
>
> :rotating_light: **版权与许可证说明**: 上游 Sub2API 仓库自 `v0.1.114` 之后已由 MIT 协议变更为 LGPL 协议。基于该变更，本仓库将不再同步上游仓库 `v0.1.114` 之后的更新。
...
通过 Nginx 反向代理 Tabro（或 CRS 服务）并搭配 Codex CLI 使用时，需要在 Nginx 配置的 `http` 块中添加：
...
Tabro 支持 [Antigravity](https://antigravity.so/) 账户，授权后可通过专用端点访问 Claude 和 Gemini 模型。
